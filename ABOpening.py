...
# The brain of the AI.
def ABOpening(boardString, depth, alpha, beta, isMaximizingPlayer):

    global positionsEvaluated

    if depth == 0:
        positionsEvaluated += 1
        return m.static_estimation(boardString, True, True), None
    ...
    if isMaximizingPlayer:
        ...
        for move in possible_moves:
            eval_score, _ = ABOpening(move, depth - 1, alpha, beta, False)
        ...
        for move in possible_moves:
            ...
            eval_score, _ = ABOpening(move, depth - 1, alpha, beta, True)
...
def main():
    ...
    # m.write_board_to_file(legacy, output_file)  # Ensure the output file is created
    ...
    eval_score, best_move = ABOpening(m.board_to_string(initial_board), depth, float("-inf"), float("inf"), True)
